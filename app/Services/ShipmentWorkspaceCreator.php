...
class ShipmentWorkspaceCreator
{
    ...
    private function checklist(string $transportType): array
    {
        $items = match ($transportType) {
            ...
            'sea' => ['booking_confirmation', 'shipping_line_and_agent', 'container_details', 'vessel_and_voyage', 'shipping_instructions', 'approve_shipping_instructions', 'draft_bill_of_lading', 'approve_draft', 'final_bill_of_lading'],
        ...
        return array_map(fn (string $key): array => [
            ...
            'waiting_for_status' => ChecklistStatusRequirements::waitingForStatus(['key' => $key]),
            'status' => 'pending',
            'action_value' => null,
            'completed_at' => null,
