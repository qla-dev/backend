...
class LoadController extends CrudController
{
    ...
    {
        return ['customer.role', 'consignee.user.role', 'company', 'assignedDriver.driver', 'vehicle.latestLocation', 'stops', 'offers', 'shipment.events', 'shipmentWorkspace.acceptedOffer.warehouse', 'routes.stops', 'notes.author', 'documents'];
    }
    ...
    protected function relationsForRequest(Request $request): array
    {
        if ($request->boolean('tracking')) {
            ...
            return ['consignee', 'company', 'assignedDriver.driver', 'vehicle.latestLocation', 'stops', 'shipment', 'shipmentWorkspace.acceptedOffer.warehouse'];
        }
    ...
    protected function rules(bool $updating = false, array $payload = []): array
    {
        ...
        return [
            ...
            'extra_packaging' => ['nullable', 'array', 'max:20'],
            'extra_packaging.*.pallets' => ['nullable', 'integer', 'min:0'],
            'extra_packaging.*.quantity_measure' => ['nullable', 'string', 'max:255'],
            'extra_packaging.*.weight_kg' => ['nullable', 'numeric', 'min:0'],
            'extra_packaging.*.length_m' => ['nullable', 'numeric', 'min:0'],
            'extra_packaging.*.width_m' => ['nullable', 'numeric', 'min:0'],
            'extra_packaging.*.height_m' => ['nullable', 'numeric', 'min:0'],
            'extra_packaging.*.volume_m3' => ['nullable', 'numeric', 'min:0'],
            'extra_packaging.*.dimension_scope' => ['nullable', 'in:overall,per_unit'],
            ...
            'contact.supplier' => [$updating ? 'sometimes' : 'required', 'array'],
            'contact.supplier.name' => [$updating ? 'required_with:contact.supplier' : 'required', 'string', 'max:255'],
            'contact.supplier.email' => [$updating ? 'required_with:contact.supplier' : 'required', 'email', 'max:255'],
            'contact.supplier.phone' => [$updating ? 'required_with:contact.supplier' : 'required', 'string', 'max:50'],
            'contact.supplier.mobile' => ['nullable', 'string', 'max:50'],
            'contact.supplier.fax' => ['nullable', 'string', 'max:50'],
            'notes' => ['nullable', 'string'], 'internal_comments' => ['nullable', 'string'],
