...
class ConversationController extends CrudController
{
    ...
    protected function rules(bool $u = false): array
    {
        ...
        return ['company_id' => ['nullable', 'integer', 'exists:companies,id'], 'load_id' => ['nullable', 'integer', 'exists:loads,id'], 'load_draft_id' => ['nullable', 'integer', 'exists:load_drafts,id'], 'created_by_user_id' => [$p, 'integer', 'exists:users,id'], 'channel' => ['sometimes', 'in:inapp,whatsapp,telegram'], 'subject' => ['nullable', 'string', 'max:255'], 'canvas' => ['sometimes', 'boolean'], 'last_message_at' => ['nullable', 'date'], 'participant_ids' => ['sometimes', 'array'], 'participant_ids.*' => ['integer', 'exists:users,id'], 'initial_message' => ['nullable', 'string', 'max:2000'], 'greeting' => ['nullable', 'in:draft,warehouse_transport,last_mile'], 'lang' => ['nullable', 'in:en,de,bs,hr,sr']];
    }
