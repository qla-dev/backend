...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $offer = Offer::query()->firstOrCreate(['load_id' => $load->id, 'created_by_user_id' => $users['company_demo']->id], ['company_id' => $company->id, 'driver_user_id' => $users['driver_demo']->id, 'amount' => 1480, 'currency' => 'EUR', 'status' => 'accepted']);
        if (! $load->shipmentWorkspace()->exists()) {
            app(\App\Services\ShipmentWorkspaceCreator::class)->create($load, $offer, $users['company_demo']);
        }
        $workspace = $load->shipmentWorkspace()->firstOrFail();
        $workspace->update(['operational_checklist' => array_map(fn (array $item): array => [...$item, 'status' => 'completed', 'completed_at' => now()->toIso8601String(), 'completed_by_user_id' => $users['company_demo']->id], $workspace->operational_checklist)]);
