...
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach ($this->keys() as [$table, $column, $parent, , $nullable]) {
            ...
            if ($nullable) $query->whereNotNull("c.{$column}");
            if ($count = $query->count()) $invalid["{$table}.{$column} -> {$parent}.id"] = $count;
        }
        if ($invalid) throw new RuntimeException('Foreign-key repair stopped; no schema changed. Invalid references: '.collect($invalid)->map(fn($n,$k)=>"$k: $n")->implode(', '));

        foreach (collect($this->keys())->flatMap(fn($k)=>[$k[0],$k[2]])->unique()->sort() as $table) {
            DB::statement("ALTER TABLE `{$table}` ENGINE=InnoDB");
        }
        foreach ($this->keys() as [$table,$column,$parent,$delete]) $this->add($table,$column,$parent,$delete);
    }
    ...
    private function keys(): array
    {
        return array_map(function ($line) {
            [$t,$c,$p,$d,$n] = explode('|',$line); return [$t,$c,$p,$d,$n === '1'];
        }, [
    ...
    {
        if (DB::table('information_schema.KEY_COLUMN_USAGE')->where('TABLE_SCHEMA',DB::getDatabaseName())->where('TABLE_NAME',$table)->where('COLUMN_NAME',$column)->where('REFERENCED_TABLE_NAME',$parent)->exists()) return;
        $name=$this->name($table,$column);
    ...
    private function name(string $table,string $column): string { return "fk_{$table}_{$column}"; }
};
